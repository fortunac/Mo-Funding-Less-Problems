  ...
  <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
            ...
            <ul class="nav navbar-nav navbar-links style-4 clearfix">
              ...
              <li>
                ...
                <form action = "results.php" method = "get">
                  <input type = "search" name="searchparameter" class = "searchbar" id = "searchparameter" placeholder=" Basic Search">
      ...
      <header class="intro">
        ...
        $title = $_GET["project-name"];
        $duration = $_GET["project-duration"];
        $person = $_GET["project-host"];
        $end = $_GET["project-end"];
        $categoryOne = $_GET["project-category1"];
        $categoryTwo = $_GET["project-category2"];
        $categoryThree = $_GET["project-category3"];
        $description = $_GET["project-description"];

        $query = "SELECT pr.image, pr.title, pr.duration, pr.currentAmount, pr.goalAmount - pr.currentAmount, pr.id
        FROM project pr, projectPerson pp, person pe
        WHERE LOWER(pr.title) LIKE LOWER('%" . $title . "%')
        AND LOWER(pr.description) LIKE LOWER('%" . $description . "%')
        AND LOWER(pe.name) LIKE LOWER('%" . $person . "%')
        AND pr.id = pp.project
        AND pp.person = pe.email
        AND pr.id IN (
          SELECT pr.id
          FROM project pr, projectCategory pc1, projectCategory pc2, projectCategory pc3, category c1, category c2, category c3
          WHERE LOWER(c1.name) LIKE LOWER('%" . $categoryOne . "%')
          AND LOWER(c2.name) LIKE LOWER('%" . $categoryTwo . "%')
          AND LOWER(c3.name) LIKE LOWER('%" . $categoryThree . "%')
          AND pr.id = pc1.project
          AND pr.id = pc2.project
          AND pr.id = pc3.project
          AND pc1.category = c1.name
          AND pc2.category = c2.name
          AND pc3.category = c3.name)
